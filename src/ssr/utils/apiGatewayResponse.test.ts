...
it('can correctly set parameters', () => {
  ...
  expect((headers as any).foofoo).toBe('barbar')
  expect((options as any).foo).toBe('bar')
})

it('can override headers', () => {
  const response: any = makeResponse(
    testRequest,
    (error, { body, headers }) => {
...
it('CSP headers set correctly', () => {
  const response: any = makeResponse(testRequest, (error, { headers }) => {
    expect(error).toBeNull()
    ...
    expect(headers['report-to']).toBeTruthy()
    expect(JSON.parse(headers['report-to']).endpoints[0].url).toBeTruthy()
  })
